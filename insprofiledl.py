...
def Scap(ssid):
	...
	if profile_name == '':
		print('------ Empty ! enter username again ')
		Scap(ssid)
	profile_headers = {'user-agent':USER_AGENT,
					   'cookie':'sessionid=' + ssid}
	print('------------- Sending request.... -------------')
	profile_resp = requests.get(BASE_URL + f"/{profile_name}/" , headers = profile_headers)
		...
		if(not resp):
			...
			if un_login(resp):
				...
				if len(USER_DICT['edge_owner_to_timeline_media']['edges'] ) <= 0:
					...
					print(f"Counting all page of profile @{USER_DICT['full_name']}....")
...
def parse__json(isFirst , json):
	...
	try:
		...
		print('--- ERROR : invalid JSON or response is not JSON ---')
